...
func (xi *Items) String() string {
	s := fmt.Sprintf("%-36s %-36s %-40s %-11s %-11s %s", "UUID", "TRANSACTION UUID", "DESCRIPTION", "DEBIT", "CREDIT", "TAGS")
	for _, i := range *xi {
		desc := i.Description
		if len(i.Description) > 40 {
			desc = i.Description[:41]
		}
		s += fmt.Sprintf("%36s %36s %-40s %11.2f %11.2f %s\n", i.UUID, i.TransactionUUID, desc, i.Debit, i.Credit, i.Tags)
	}
	return s
}
